...
namespace XCache
{
    ...
    public:
        XLRUCache(int capacity) : capacity(capacity)
        {
            initializeList();
        }
    ...
    template <typename Key, typename Value>
    class XLRUKCache : public XLRUCache<Key, Value> // LRU优化的K版本，只有在历史访问次数达到K次时，才会将节点移动到主缓存中
    {
    public:
        XLRUKCache(int capacity, int history, int _k)
            : XLRUCache<Key, Value>(capacity),                               // 初始化主缓存的容量
              historyList(std::make_unique<XLRUCache<Key, size_t>>(history)), // 初始化历史缓存的容量
              k(_k)
        {
        }
        ...
        Value get(Key key)
        {
            ...
            if (historycount >= k) // 如果历史访问次数达到K次，将节点移动到主缓存中
            {
                ...
                if (it != historyMap.end())
                {
                    ...
                    historyMap.erase(it);
                    XLRUCache<Key, Value>::put(key, it->second);
                    return it->second;
                }
            }
